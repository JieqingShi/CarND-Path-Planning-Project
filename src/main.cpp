      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for(int i = 0; i < path_size-previous_path_x.size(); i++){  // assuming the path always consists of path_size points
            ...
            else if(target_vel < 0.224){
              target_vel = 0.224;
            }
            double N = target_dist/(0.02*target_vel/2.24); // Number of points for splitting up the trajectory along the target distance; converting from mph to m/sec, evaluating new point every 20 ms
